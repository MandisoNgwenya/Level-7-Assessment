<template>
  ...
  <v-card class="mt-8">
    <v-col cols="12" md="12">
      <v-card-title> <h1>Comments</h1></v-card-title>
    </v-col>
    <v-col
      v-if="comments"
    ...
    >
      <v-card>
        <v-card-text>
          <v-textarea rows="2" v-model="comment.comment"></v-textarea>
...
export default {
  ...
  methods: {
    ...
    async deleteComment(comment) {
      ...
      this.comments = await $fetch("http://127.0.0.1:8000/api/comment/delete", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
